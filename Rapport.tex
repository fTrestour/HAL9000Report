    ...
    \part[Le Perceptron]{Première approche du problème : le Perceptron}
        ...
        \chapter{Algorithmes d'apprentissage}
            ...
            \section{La méthode de rétropropagation du gradient}
                ...
                La rétropropagation du gradient correspond à une descente de gradient dans l'espace des poids. On pourra aussi désigner cette méthode sous son nom anglais : \textit{backpropagation}\\
                ...
                \subsection{Potentielle modification de l'algorithme}
                    ...
                    Cette technique permet d'éviter certains minima locaux et de se stabiliser dans des configurations plus optimales. Cela peut aussi accélérer la convergence de l'apprentissage si le coefficient $\alpha$ est bien choisi.
    ...
    \part[Machines de Boltzmann]{Approfondir le sujet : les machines de Boltzmann.}
        ...
        \chapter{Les machines de Boltzmann restreintes}
            ...
            \section{Présentation et éléments de preuve}
                ...
                couche visible soit activé en connaissant l'état des neurones $h_{j}$ de la couche cachée \textit{H} est \begin{equation}p( v_{i} =1 \mid H) = 
...
à \textit{$h_{j}$} et \textit{$b_{i}$} le biais du neurone \textit{$v_{i}$}.\\
...
sont atteignables en un nombre fini d'échantillonnage ce qui montre que la chaîne est irréductible. De plus la chaîne de Markov est apériodique . 
...
\begin{figure}[!ht]
                    \begin{center}
                        \includegraphics[width=175pt]{Images/Gibbs.png}
                    \end{center}
...
Hinton propose d'ailleurs de ne faire qu'un seul pas de Gibbs (considérer que $<v_{i}h_{j}>_{model}$ est atteint à $t=1$ dans la figure 5.1).
...
\section{Principe des Deep Boltzmann Machines}
		...
		Nous verrons dans ce chapitre une nouvelle façon d'utiliser ces machines, en série. Après la description d'une configuration intéressante pour l'apprentissage, nous présenterons nos résultats obtenus sur la base MNIST.\\
